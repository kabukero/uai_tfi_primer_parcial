	...
	public class LLamadaAdapter
	{
		public static Llamada Adapt(object[] values)
		{
			...
			return new Llamada()
			{
				...
				TelefonoOrigen = repoTelefono.GetOne(Guid.Parse(values[(int)Columnas.TELEFONO_ORIGEN_ID].ToString())),
				DestinoOrigen = repoDestino.GetOne(Guid.Parse(values[(int)Columnas.DESTINO_ORIGEN_ID].ToString())),
